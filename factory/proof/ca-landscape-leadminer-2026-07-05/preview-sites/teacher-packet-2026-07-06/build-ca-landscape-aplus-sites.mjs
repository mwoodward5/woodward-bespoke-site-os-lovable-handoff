...
function home(packet, spec, images, video, logoUrl, lane) {
  ...
  const body = lane === "single-page-cinematic"
    ? `${hero(packet, spec, images, video)}${trust(packet)}${services(packet, spec)}${founderBlock(packet, spec, images)}${proofBand(packet, spec)}${processRibbon(packet)}${geoMapBand(packet, spec)}${faqSection(packet, spec)}${quote(packet, spec)}${footer(packet)}`
    : `${hero(packet, spec, images, video)}${trust(packet)}<section class="section"><div class="shell premier-intro"><div class="premier-copy"><p class="kicker">Premier authority hub</p><h2>A deeper site for search, service areas, and real customer decisions.</h2><p>This version keeps the cinematic homepage but adds proper inner pages for services, process, service areas, FAQ, contact, and source review so the business can grow beyond a single scroll.</p></div><div class="premier-copy"><h2>${esc(packet.business.city)} map-first trust.</h2><p>The map/directions flow becomes its own page, with satellite context, service-area pins, and a cleaner route for homeowners who are already deciding whether the company is local enough.</p></div></div></section>${services(packet, spec)}${geoMapBand(packet, spec)}${gallery(packet, spec, images)}${quote(packet, spec)}${footer(packet)}`;
  return `${head(packet, spec, title, description)}
<body><a class="skip" href="#main">Skip to content</a><div class="texture"></div><div class="site ${spec.theme}" data-kitchen-stack="master-glue,razzle-fx,site-superpowers" data-lane="${lane}">${nav(packet, spec, logoUrl, lane)}${body}</div></body></html>`;
}
...
function writeSite(packet, index) {
  ...
  writeFileSync(path.join(dir, "styles.css"), css(spec) + formulaCss() + heroCompositionCss());
