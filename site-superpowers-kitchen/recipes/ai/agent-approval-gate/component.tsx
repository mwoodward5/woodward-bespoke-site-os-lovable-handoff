...
/**
 * Agent Approval Gate — AI recipe
 * Talks to /api/agent-approval-gate (server route) which proxies Lovable AI Gateway.
 */
export function SskAgentApprovalGate() {
  const [input, setInput] = useState("");
  const [out, setOut] = useState("");
  const [busy, setBusy] = useState(false);
  async function send() {
    setBusy(true); setOut("");
    const res = await fetch("/api/agent-approval-gate", {
...
export default SskAgentApprovalGate;
