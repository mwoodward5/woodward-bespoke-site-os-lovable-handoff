...
/**
 * Agent Cost Cap — AI recipe
 * Talks to /api/agent-cost-cap (server route) which proxies Lovable AI Gateway.
 */
export function SskAgentCostCap() {
  const [input, setInput] = useState("");
  const [out, setOut] = useState("");
  const [busy, setBusy] = useState(false);
  async function send() {
    setBusy(true); setOut("");
    const res = await fetch("/api/agent-cost-cap", {
...
export default SskAgentCostCap;
