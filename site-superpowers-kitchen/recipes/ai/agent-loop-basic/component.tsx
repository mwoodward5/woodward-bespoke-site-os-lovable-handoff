...
/**
 * Agent Loop Basic — AI recipe
 * Talks to /api/agent-loop-basic (server route) which proxies Lovable AI Gateway.
 */
export function SskAgentLoopBasic() {
  const [input, setInput] = useState("");
  const [out, setOut] = useState("");
  const [busy, setBusy] = useState(false);
  async function send() {
    setBusy(true); setOut("");
    const res = await fetch("/api/agent-loop-basic", {
...
export default SskAgentLoopBasic;
