...
/**
 * Agent Mcp Oauth — AI recipe
 * Talks to /api/agent-mcp-oauth (server route) which proxies Lovable AI Gateway.
 */
export function SskAgentMcpOauth() {
  const [input, setInput] = useState("");
  const [out, setOut] = useState("");
  const [busy, setBusy] = useState(false);
  async function send() {
    setBusy(true); setOut("");
    const res = await fetch("/api/agent-mcp-oauth", {
...
export default SskAgentMcpOauth;
