...
/**
 * Agent Timeout — AI recipe
 * Talks to /api/agent-timeout (server route) which proxies Lovable AI Gateway.
 */
export function SskAgentTimeout() {
  const [input, setInput] = useState("");
  const [out, setOut] = useState("");
  const [busy, setBusy] = useState(false);
  async function send() {
    setBusy(true); setOut("");
    const res = await fetch("/api/agent-timeout", {
...
export default SskAgentTimeout;
