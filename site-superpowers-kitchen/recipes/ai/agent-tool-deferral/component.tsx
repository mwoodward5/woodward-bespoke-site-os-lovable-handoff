...
/**
 * Agent Tool Deferral — AI recipe
 * Talks to /api/agent-tool-deferral (server route) which proxies Lovable AI Gateway.
 */
export function SskAgentToolDeferral() {
  const [input, setInput] = useState("");
  const [out, setOut] = useState("");
  const [busy, setBusy] = useState(false);
  async function send() {
    setBusy(true); setOut("");
    const res = await fetch("/api/agent-tool-deferral", {
...
export default SskAgentToolDeferral;
