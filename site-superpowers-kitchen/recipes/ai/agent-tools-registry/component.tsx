...
/**
 * Agent Tools Registry — AI recipe
 * Talks to /api/agent-tools-registry (server route) which proxies Lovable AI Gateway.
 */
export function SskAgentToolsRegistry() {
  const [input, setInput] = useState("");
  const [out, setOut] = useState("");
  const [busy, setBusy] = useState(false);
  async function send() {
    setBusy(true); setOut("");
    const res = await fetch("/api/agent-tools-registry", {
...
export default SskAgentToolsRegistry;
