...
/**
 * Chat Agent Mcp — AI recipe
 * Talks to /api/chat-agent-mcp (server route) which proxies Lovable AI Gateway.
 */
export function SskChatAgentMcp() {
  const [input, setInput] = useState("");
  const [out, setOut] = useState("");
  const [busy, setBusy] = useState(false);
  async function send() {
    setBusy(true); setOut("");
    const res = await fetch("/api/chat-agent-mcp", {
...
export default SskChatAgentMcp;
