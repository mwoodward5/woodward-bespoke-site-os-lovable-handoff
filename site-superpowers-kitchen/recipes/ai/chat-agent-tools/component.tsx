...
/**
 * Chat Agent Tools — AI recipe
 * Talks to /api/chat-agent-tools (server route) which proxies Lovable AI Gateway.
 */
export function SskChatAgentTools() {
  const [input, setInput] = useState("");
  const [out, setOut] = useState("");
  const [busy, setBusy] = useState(false);
  async function send() {
    setBusy(true); setOut("");
    const res = await fetch("/api/chat-agent-tools", {
...
export default SskChatAgentTools;
