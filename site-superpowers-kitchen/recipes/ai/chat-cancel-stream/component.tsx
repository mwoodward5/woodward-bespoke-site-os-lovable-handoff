...
/**
 * Chat Cancel Stream — AI recipe
 * Talks to /api/chat-cancel-stream (server route) which proxies Lovable AI Gateway.
 */
export function SskChatCancelStream() {
  const [input, setInput] = useState("");
  const [out, setOut] = useState("");
  const [busy, setBusy] = useState(false);
  async function send() {
    setBusy(true); setOut("");
    const res = await fetch("/api/chat-cancel-stream", {
...
export default SskChatCancelStream;
