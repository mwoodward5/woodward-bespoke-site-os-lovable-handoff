...
/**
 * Chat Cost Meter — AI recipe
 * Talks to /api/chat-cost-meter (server route) which proxies Lovable AI Gateway.
 */
export function SskChatCostMeter() {
  const [input, setInput] = useState("");
  const [out, setOut] = useState("");
  const [busy, setBusy] = useState(false);
  async function send() {
    setBusy(true); setOut("");
    const res = await fetch("/api/chat-cost-meter", {
...
export default SskChatCostMeter;
