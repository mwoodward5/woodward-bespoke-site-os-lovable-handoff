...
/**
 * Chat Docs Qa — AI recipe
 * Talks to /api/chat-docs-qa (server route) which proxies Lovable AI Gateway.
 */
export function SskChatDocsQa() {
  const [input, setInput] = useState("");
  const [out, setOut] = useState("");
  const [busy, setBusy] = useState(false);
  async function send() {
    setBusy(true); setOut("");
    const res = await fetch("/api/chat-docs-qa", {
