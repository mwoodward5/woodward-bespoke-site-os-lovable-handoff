...
/**
 * Chat Multilingual — AI recipe
 * Talks to /api/chat-multilingual (server route) which proxies Lovable AI Gateway.
 */
export function SskChatMultilingual() {
  const [input, setInput] = useState("");
  const [out, setOut] = useState("");
  const [busy, setBusy] = useState(false);
  async function send() {
    setBusy(true); setOut("");
    const res = await fetch("/api/chat-multilingual", {
...
export default SskChatMultilingual;
