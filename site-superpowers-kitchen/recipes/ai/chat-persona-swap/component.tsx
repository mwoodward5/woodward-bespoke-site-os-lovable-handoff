...
/**
 * Chat Persona Swap — AI recipe
 * Talks to /api/chat-persona-swap (server route) which proxies Lovable AI Gateway.
 */
export function SskChatPersonaSwap() {
  const [input, setInput] = useState("");
  const [out, setOut] = useState("");
  const [busy, setBusy] = useState(false);
  async function send() {
    setBusy(true); setOut("");
    const res = await fetch("/api/chat-persona-swap", {
...
export default SskChatPersonaSwap;
