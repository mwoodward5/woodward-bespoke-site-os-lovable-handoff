...
/**
 * Chat Pii Redact — AI recipe
 * Talks to /api/chat-pii-redact (server route) which proxies Lovable AI Gateway.
 */
export function SskChatPiiRedact() {
  const [input, setInput] = useState("");
  const [out, setOut] = useState("");
  const [busy, setBusy] = useState(false);
  async function send() {
    setBusy(true); setOut("");
    const res = await fetch("/api/chat-pii-redact", {
...
export default SskChatPiiRedact;
