...
/**
 * Chat Rag Basic — AI recipe
 * Talks to /api/chat-rag-basic (server route) which proxies Lovable AI Gateway.
 */
export function SskChatRagBasic() {
  const [input, setInput] = useState("");
  const [out, setOut] = useState("");
  const [busy, setBusy] = useState(false);
  async function send() {
    setBusy(true); setOut("");
    const res = await fetch("/api/chat-rag-basic", {
...
export default SskChatRagBasic;
