...
/**
 * Chat Rag Hybrid — AI recipe
 * Talks to /api/chat-rag-hybrid (server route) which proxies Lovable AI Gateway.
 */
export function SskChatRagHybrid() {
  const [input, setInput] = useState("");
  const [out, setOut] = useState("");
  const [busy, setBusy] = useState(false);
  async function send() {
    setBusy(true); setOut("");
    const res = await fetch("/api/chat-rag-hybrid", {
...
export default SskChatRagHybrid;
