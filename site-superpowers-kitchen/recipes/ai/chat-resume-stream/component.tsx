...
/**
 * Chat Resume Stream — AI recipe
 * Talks to /api/chat-resume-stream (server route) which proxies Lovable AI Gateway.
 */
export function SskChatResumeStream() {
  const [input, setInput] = useState("");
  const [out, setOut] = useState("");
  const [busy, setBusy] = useState(false);
  async function send() {
    setBusy(true); setOut("");
    const res = await fetch("/api/chat-resume-stream", {
...
export default SskChatResumeStream;
