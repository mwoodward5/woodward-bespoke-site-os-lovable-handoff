...
/**
 * Chat Sales Bot — AI recipe
 * Talks to /api/chat-sales-bot (server route) which proxies Lovable AI Gateway.
 */
export function SskChatSalesBot() {
  const [input, setInput] = useState("");
  const [out, setOut] = useState("");
  const [busy, setBusy] = useState(false);
  async function send() {
    setBusy(true); setOut("");
    const res = await fetch("/api/chat-sales-bot", {
...
export default SskChatSalesBot;
