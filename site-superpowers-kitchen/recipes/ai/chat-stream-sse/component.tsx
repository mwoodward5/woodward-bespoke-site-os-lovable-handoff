...
/**
 * Chat Stream Sse — AI recipe
 * Talks to /api/chat-stream-sse (server route) which proxies Lovable AI Gateway.
 */
export function SskChatStreamSse() {
  const [input, setInput] = useState("");
  const [out, setOut] = useState("");
  const [busy, setBusy] = useState(false);
  async function send() {
    setBusy(true); setOut("");
    const res = await fetch("/api/chat-stream-sse", {
...
export default SskChatStreamSse;
