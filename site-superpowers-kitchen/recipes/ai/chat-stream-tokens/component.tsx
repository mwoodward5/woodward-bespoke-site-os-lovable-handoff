...
/**
 * Chat Stream Tokens — AI recipe
 * Talks to /api/chat-stream-tokens (server route) which proxies Lovable AI Gateway.
 */
export function SskChatStreamTokens() {
  const [input, setInput] = useState("");
  const [out, setOut] = useState("");
  const [busy, setBusy] = useState(false);
  async function send() {
    setBusy(true); setOut("");
    const res = await fetch("/api/chat-stream-tokens", {
...
export default SskChatStreamTokens;
