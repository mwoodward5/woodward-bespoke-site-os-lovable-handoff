...
/**
 * Chat Summarizer — AI recipe
 * Talks to /api/chat-summarizer (server route) which proxies Lovable AI Gateway.
 */
export function SskChatSummarizer() {
  const [input, setInput] = useState("");
  const [out, setOut] = useState("");
  const [busy, setBusy] = useState(false);
  async function send() {
    setBusy(true); setOut("");
    const res = await fetch("/api/chat-summarizer", {
...
export default SskChatSummarizer;
