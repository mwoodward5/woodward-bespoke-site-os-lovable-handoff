...
/**
 * Chat Support Bot — AI recipe
 * Talks to /api/chat-support-bot (server route) which proxies Lovable AI Gateway.
 */
export function SskChatSupportBot() {
  const [input, setInput] = useState("");
  const [out, setOut] = useState("");
  const [busy, setBusy] = useState(false);
  async function send() {
    setBusy(true); setOut("");
    const res = await fetch("/api/chat-support-bot", {
...
export default SskChatSupportBot;
