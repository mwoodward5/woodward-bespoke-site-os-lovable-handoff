...
/**
 * Chat Tone Shifter — AI recipe
 * Talks to /api/chat-tone-shifter (server route) which proxies Lovable AI Gateway.
 */
export function SskChatToneShifter() {
  const [input, setInput] = useState("");
  const [out, setOut] = useState("");
  const [busy, setBusy] = useState(false);
  async function send() {
    setBusy(true); setOut("");
    const res = await fetch("/api/chat-tone-shifter", {
...
export default SskChatToneShifter;
