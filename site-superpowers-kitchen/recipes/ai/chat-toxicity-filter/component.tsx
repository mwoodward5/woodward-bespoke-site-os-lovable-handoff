...
/**
 * Chat Toxicity Filter — AI recipe
 * Talks to /api/chat-toxicity-filter (server route) which proxies Lovable AI Gateway.
 */
export function SskChatToxicityFilter() {
  const [input, setInput] = useState("");
  const [out, setOut] = useState("");
  const [busy, setBusy] = useState(false);
  async function send() {
    setBusy(true); setOut("");
    const res = await fetch("/api/chat-toxicity-filter", {
...
export default SskChatToxicityFilter;
