...
/**
 * Chat Translator — AI recipe
 * Talks to /api/chat-translator (server route) which proxies Lovable AI Gateway.
 */
export function SskChatTranslator() {
  const [input, setInput] = useState("");
  const [out, setOut] = useState("");
  const [busy, setBusy] = useState(false);
  async function send() {
    setBusy(true); setOut("");
    const res = await fetch("/api/chat-translator", {
...
export default SskChatTranslator;
