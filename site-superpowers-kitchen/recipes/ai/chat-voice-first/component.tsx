...
/**
 * Chat Voice First — AI recipe
 * Talks to /api/chat-voice-first (server route) which proxies Lovable AI Gateway.
 */
export function SskChatVoiceFirst() {
  const [input, setInput] = useState("");
  const [out, setOut] = useState("");
  const [busy, setBusy] = useState(false);
  async function send() {
    setBusy(true); setOut("");
    const res = await fetch("/api/chat-voice-first", {
...
export default SskChatVoiceFirst;
