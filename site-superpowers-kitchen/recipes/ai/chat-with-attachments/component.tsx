...
/**
 * Chat With Attachments — AI recipe
 * Talks to /api/chat-with-attachments (server route) which proxies Lovable AI Gateway.
 */
export function SskChatWithAttachments() {
  const [input, setInput] = useState("");
  const [out, setOut] = useState("");
  const [busy, setBusy] = useState(false);
  async function send() {
    setBusy(true); setOut("");
    const res = await fetch("/api/chat-with-attachments", {
...
export default SskChatWithAttachments;
