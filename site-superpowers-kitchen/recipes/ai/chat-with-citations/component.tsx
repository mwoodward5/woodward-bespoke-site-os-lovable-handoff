...
/**
 * Chat With Citations — AI recipe
 * Talks to /api/chat-with-citations (server route) which proxies Lovable AI Gateway.
 */
export function SskChatWithCitations() {
  const [input, setInput] = useState("");
  const [out, setOut] = useState("");
  const [busy, setBusy] = useState(false);
  async function send() {
    setBusy(true); setOut("");
    const res = await fetch("/api/chat-with-citations", {
...
export default SskChatWithCitations;
