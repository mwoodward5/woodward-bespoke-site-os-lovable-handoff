...
/**
 * Chat With Images — AI recipe
 * Talks to /api/chat-with-images (server route) which proxies Lovable AI Gateway.
 */
export function SskChatWithImages() {
  const [input, setInput] = useState("");
  const [out, setOut] = useState("");
  const [busy, setBusy] = useState(false);
  async function send() {
    setBusy(true); setOut("");
    const res = await fetch("/api/chat-with-images", {
...
export default SskChatWithImages;
