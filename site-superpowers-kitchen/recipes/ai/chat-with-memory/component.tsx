...
/**
 * Chat With Memory — AI recipe
 * Talks to /api/chat-with-memory (server route) which proxies Lovable AI Gateway.
 */
export function SskChatWithMemory() {
  const [input, setInput] = useState("");
  const [out, setOut] = useState("");
  const [busy, setBusy] = useState(false);
  async function send() {
    setBusy(true); setOut("");
    const res = await fetch("/api/chat-with-memory", {
...
export default SskChatWithMemory;
