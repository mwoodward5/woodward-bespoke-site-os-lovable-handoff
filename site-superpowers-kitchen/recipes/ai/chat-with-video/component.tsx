...
/**
 * Chat With Video — AI recipe
 * Talks to /api/chat-with-video (server route) which proxies Lovable AI Gateway.
 */
export function SskChatWithVideo() {
  const [input, setInput] = useState("");
  const [out, setOut] = useState("");
  const [busy, setBusy] = useState(false);
  async function send() {
    setBusy(true); setOut("");
    const res = await fetch("/api/chat-with-video", {
...
export default SskChatWithVideo;
