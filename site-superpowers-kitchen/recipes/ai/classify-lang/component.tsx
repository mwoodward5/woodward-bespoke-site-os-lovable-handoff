...
/**
 * Classify Lang — AI recipe
 * Talks to /api/classify-lang (server route) which proxies Lovable AI Gateway.
 */
export function SskClassifyLang() {
  const [input, setInput] = useState("");
  const [out, setOut] = useState("");
  const [busy, setBusy] = useState(false);
  async function send() {
    setBusy(true); setOut("");
    const res = await fetch("/api/classify-lang", {
...
export default SskClassifyLang;
