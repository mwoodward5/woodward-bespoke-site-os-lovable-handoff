...
/**
 * Completion Alt Text — AI recipe
 * Talks to /api/completion-alt-text (server route) which proxies Lovable AI Gateway.
 */
export function SskCompletionAltText() {
  const [input, setInput] = useState("");
  const [out, setOut] = useState("");
  const [busy, setBusy] = useState(false);
  async function send() {
    setBusy(true); setOut("");
    const res = await fetch("/api/completion-alt-text", {
...
export default SskCompletionAltText;
