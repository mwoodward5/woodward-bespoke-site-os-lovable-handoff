...
/**
 * Completion Changelog — AI recipe
 * Talks to /api/completion-changelog (server route) which proxies Lovable AI Gateway.
 */
export function SskCompletionChangelog() {
  const [input, setInput] = useState("");
  const [out, setOut] = useState("");
  const [busy, setBusy] = useState(false);
  async function send() {
    setBusy(true); setOut("");
    const res = await fetch("/api/completion-changelog", {
...
export default SskCompletionChangelog;
