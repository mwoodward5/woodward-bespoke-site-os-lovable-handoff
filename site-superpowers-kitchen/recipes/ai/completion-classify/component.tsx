...
/**
 * Completion Classify — AI recipe
 * Talks to /api/completion-classify (server route) which proxies Lovable AI Gateway.
 */
export function SskCompletionClassify() {
  const [input, setInput] = useState("");
  const [out, setOut] = useState("");
  const [busy, setBusy] = useState(false);
  async function send() {
    setBusy(true); setOut("");
    const res = await fetch("/api/completion-classify", {
...
export default SskCompletionClassify;
