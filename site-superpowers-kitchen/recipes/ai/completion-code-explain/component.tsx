...
/**
 * Completion Code Explain — AI recipe
 * Talks to /api/completion-code-explain (server route) which proxies Lovable AI Gateway.
 */
export function SskCompletionCodeExplain() {
  const [input, setInput] = useState("");
  const [out, setOut] = useState("");
  const [busy, setBusy] = useState(false);
  async function send() {
    setBusy(true); setOut("");
    const res = await fetch("/api/completion-code-explain", {
...
export default SskCompletionCodeExplain;
