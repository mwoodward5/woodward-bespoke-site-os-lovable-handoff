...
/**
 * Completion Code Refactor — AI recipe
 * Talks to /api/completion-code-refactor (server route) which proxies Lovable AI Gateway.
 */
export function SskCompletionCodeRefactor() {
  const [input, setInput] = useState("");
  const [out, setOut] = useState("");
  const [busy, setBusy] = useState(false);
  async function send() {
    setBusy(true); setOut("");
    const res = await fetch("/api/completion-code-refactor", {
...
export default SskCompletionCodeRefactor;
