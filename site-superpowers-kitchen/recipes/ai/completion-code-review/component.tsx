...
/**
 * Completion Code Review — AI recipe
 * Talks to /api/completion-code-review (server route) which proxies Lovable AI Gateway.
 */
export function SskCompletionCodeReview() {
  const [input, setInput] = useState("");
  const [out, setOut] = useState("");
  const [busy, setBusy] = useState(false);
  async function send() {
    setBusy(true); setOut("");
    const res = await fetch("/api/completion-code-review", {
...
export default SskCompletionCodeReview;
