...
/**
 * Completion Code Tests — AI recipe
 * Talks to /api/completion-code-tests (server route) which proxies Lovable AI Gateway.
 */
export function SskCompletionCodeTests() {
  const [input, setInput] = useState("");
  const [out, setOut] = useState("");
  const [busy, setBusy] = useState(false);
  async function send() {
    setBusy(true); setOut("");
    const res = await fetch("/api/completion-code-tests", {
...
export default SskCompletionCodeTests;
