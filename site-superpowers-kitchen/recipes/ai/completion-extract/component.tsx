...
/**
 * Completion Extract — AI recipe
 * Talks to /api/completion-extract (server route) which proxies Lovable AI Gateway.
 */
export function SskCompletionExtract() {
  const [input, setInput] = useState("");
  const [out, setOut] = useState("");
  const [busy, setBusy] = useState(false);
  async function send() {
    setBusy(true); setOut("");
    const res = await fetch("/api/completion-extract", {
...
export default SskCompletionExtract;
