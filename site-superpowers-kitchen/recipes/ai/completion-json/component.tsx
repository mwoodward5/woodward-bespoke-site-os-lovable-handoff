...
/**
 * Completion Json — AI recipe
 * Talks to /api/completion-json (server route) which proxies Lovable AI Gateway.
 */
export function SskCompletionJson() {
  const [input, setInput] = useState("");
  const [out, setOut] = useState("");
  const [busy, setBusy] = useState(false);
  async function send() {
    setBusy(true); setOut("");
    const res = await fetch("/api/completion-json", {
...
export default SskCompletionJson;
