...
/**
 * Completion Meta Tags — AI recipe
 * Talks to /api/completion-meta-tags (server route) which proxies Lovable AI Gateway.
 */
export function SskCompletionMetaTags() {
  const [input, setInput] = useState("");
  const [out, setOut] = useState("");
  const [busy, setBusy] = useState(false);
  async function send() {
    setBusy(true); setOut("");
    const res = await fetch("/api/completion-meta-tags", {
...
export default SskCompletionMetaTags;
