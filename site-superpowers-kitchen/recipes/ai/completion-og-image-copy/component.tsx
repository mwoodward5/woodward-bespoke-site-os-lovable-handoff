...
/**
 * Completion Og Image Copy — AI recipe
 * Talks to /api/completion-og-image-copy (server route) which proxies Lovable AI Gateway.
 */
export function SskCompletionOgImageCopy() {
  const [input, setInput] = useState("");
  const [out, setOut] = useState("");
  const [busy, setBusy] = useState(false);
  async function send() {
    setBusy(true); setOut("");
    const res = await fetch("/api/completion-og-image-copy", {
...
export default SskCompletionOgImageCopy;
