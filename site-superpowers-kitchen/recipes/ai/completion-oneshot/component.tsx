...
/**
 * Completion Oneshot — AI recipe
 * Talks to /api/completion-oneshot (server route) which proxies Lovable AI Gateway.
 */
export function SskCompletionOneshot() {
  const [input, setInput] = useState("");
  const [out, setOut] = useState("");
  const [busy, setBusy] = useState(false);
  async function send() {
    setBusy(true); setOut("");
    const res = await fetch("/api/completion-oneshot", {
...
export default SskCompletionOneshot;
