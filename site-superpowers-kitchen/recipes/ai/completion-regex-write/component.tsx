...
/**
 * Completion Regex Write — AI recipe
 * Talks to /api/completion-regex-write (server route) which proxies Lovable AI Gateway.
 */
export function SskCompletionRegexWrite() {
  const [input, setInput] = useState("");
  const [out, setOut] = useState("");
  const [busy, setBusy] = useState(false);
  async function send() {
    setBusy(true); setOut("");
    const res = await fetch("/api/completion-regex-write", {
...
export default SskCompletionRegexWrite;
