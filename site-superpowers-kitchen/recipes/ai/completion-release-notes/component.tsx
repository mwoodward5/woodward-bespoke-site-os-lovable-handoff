...
/**
 * Completion Release Notes — AI recipe
 * Talks to /api/completion-release-notes (server route) which proxies Lovable AI Gateway.
 */
export function SskCompletionReleaseNotes() {
  const [input, setInput] = useState("");
  const [out, setOut] = useState("");
  const [busy, setBusy] = useState(false);
  async function send() {
    setBusy(true); setOut("");
    const res = await fetch("/api/completion-release-notes", {
...
export default SskCompletionReleaseNotes;
