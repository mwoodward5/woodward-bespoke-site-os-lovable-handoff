...
/**
 * Completion Rewrite — AI recipe
 * Talks to /api/completion-rewrite (server route) which proxies Lovable AI Gateway.
 */
export function SskCompletionRewrite() {
  const [input, setInput] = useState("");
  const [out, setOut] = useState("");
  const [busy, setBusy] = useState(false);
  async function send() {
    setBusy(true); setOut("");
    const res = await fetch("/api/completion-rewrite", {
...
export default SskCompletionRewrite;
