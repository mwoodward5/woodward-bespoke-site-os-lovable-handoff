...
/**
 * Completion Slug Suggest — AI recipe
 * Talks to /api/completion-slug-suggest (server route) which proxies Lovable AI Gateway.
 */
export function SskCompletionSlugSuggest() {
  const [input, setInput] = useState("");
  const [out, setOut] = useState("");
  const [busy, setBusy] = useState(false);
  async function send() {
    setBusy(true); setOut("");
    const res = await fetch("/api/completion-slug-suggest", {
...
export default SskCompletionSlugSuggest;
