...
/**
 * Completion Sql Write — AI recipe
 * Talks to /api/completion-sql-write (server route) which proxies Lovable AI Gateway.
 */
export function SskCompletionSqlWrite() {
  const [input, setInput] = useState("");
  const [out, setOut] = useState("");
  const [busy, setBusy] = useState(false);
  async function send() {
    setBusy(true); setOut("");
    const res = await fetch("/api/completion-sql-write", {
...
export default SskCompletionSqlWrite;
