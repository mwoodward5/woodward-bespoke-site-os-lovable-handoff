...
/**
 * Completion Structured — AI recipe
 * Talks to /api/completion-structured (server route) which proxies Lovable AI Gateway.
 */
export function SskCompletionStructured() {
  const [input, setInput] = useState("");
  const [out, setOut] = useState("");
  const [busy, setBusy] = useState(false);
  async function send() {
    setBusy(true); setOut("");
    const res = await fetch("/api/completion-structured", {
...
export default SskCompletionStructured;
