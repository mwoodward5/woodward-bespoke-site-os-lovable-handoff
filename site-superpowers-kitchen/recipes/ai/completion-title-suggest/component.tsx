...
/**
 * Completion Title Suggest — AI recipe
 * Talks to /api/completion-title-suggest (server route) which proxies Lovable AI Gateway.
 */
export function SskCompletionTitleSuggest() {
  const [input, setInput] = useState("");
  const [out, setOut] = useState("");
  const [busy, setBusy] = useState(false);
  async function send() {
    setBusy(true); setOut("");
    const res = await fetch("/api/completion-title-suggest", {
...
export default SskCompletionTitleSuggest;
