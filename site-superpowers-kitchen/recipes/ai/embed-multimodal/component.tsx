...
/**
 * Embed Multimodal — AI recipe
 * Talks to /api/embed-multimodal (server route) which proxies Lovable AI Gateway.
 */
export function SskEmbedMultimodal() {
  const [input, setInput] = useState("");
  const [out, setOut] = useState("");
  const [busy, setBusy] = useState(false);
  async function send() {
    setBusy(true); setOut("");
    const res = await fetch("/api/embed-multimodal", {
...
export default SskEmbedMultimodal;
