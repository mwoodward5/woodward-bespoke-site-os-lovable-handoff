...
/**
 * Moderation Inbound — AI recipe
 * Talks to /api/moderation-inbound (server route) which proxies Lovable AI Gateway.
 */
export function SskModerationInbound() {
  const [input, setInput] = useState("");
  const [out, setOut] = useState("");
  const [busy, setBusy] = useState(false);
  async function send() {
    setBusy(true); setOut("");
    const res = await fetch("/api/moderation-inbound", {
...
export default SskModerationInbound;
