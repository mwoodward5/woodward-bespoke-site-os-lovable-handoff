...
/**
 * Moderation Outbound — AI recipe
 * Talks to /api/moderation-outbound (server route) which proxies Lovable AI Gateway.
 */
export function SskModerationOutbound() {
  const [input, setInput] = useState("");
  const [out, setOut] = useState("");
  const [busy, setBusy] = useState(false);
  async function send() {
    setBusy(true); setOut("");
    const res = await fetch("/api/moderation-outbound", {
...
export default SskModerationOutbound;
