...
/**
 * Prompt Injection Guard — AI recipe
 * Talks to /api/prompt-injection-guard (server route) which proxies Lovable AI Gateway.
 */
export function SskPromptInjectionGuard() {
  const [input, setInput] = useState("");
  const [out, setOut] = useState("");
  const [busy, setBusy] = useState(false);
  async function send() {
    setBusy(true); setOut("");
    const res = await fetch("/api/prompt-injection-guard", {
...
export default SskPromptInjectionGuard;
