...
/**
 * Rag Chunker — AI recipe
 * Talks to /api/rag-chunker (server route) which proxies Lovable AI Gateway.
 */
export function SskRagChunker() {
  const [input, setInput] = useState("");
  const [out, setOut] = useState("");
  const [busy, setBusy] = useState(false);
  async function send() {
    setBusy(true); setOut("");
    const res = await fetch("/api/rag-chunker", {
