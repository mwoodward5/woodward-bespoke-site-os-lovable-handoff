...
/**
 * Rag Context Window Guard — AI recipe
 * Talks to /api/rag-context-window-guard (server route) which proxies Lovable AI Gateway.
 */
export function SskRagContextWindowGuard() {
  const [input, setInput] = useState("");
  const [out, setOut] = useState("");
  const [busy, setBusy] = useState(false);
  async function send() {
    setBusy(true); setOut("");
    const res = await fetch("/api/rag-context-window-guard", {
...
export default SskRagContextWindowGuard;
