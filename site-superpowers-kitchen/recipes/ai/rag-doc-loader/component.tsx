...
/**
 * Rag Doc Loader — AI recipe
 * Talks to /api/rag-doc-loader (server route) which proxies Lovable AI Gateway.
 */
export function SskRagDocLoader() {
  const [input, setInput] = useState("");
  const [out, setOut] = useState("");
  const [busy, setBusy] = useState(false);
  async function send() {
    setBusy(true); setOut("");
    const res = await fetch("/api/rag-doc-loader", {
...
export default SskRagDocLoader;
