...
/**
 * Rag Reranker — AI recipe
 * Talks to /api/rag-reranker (server route) which proxies Lovable AI Gateway.
 */
export function SskRagReranker() {
  const [input, setInput] = useState("");
  const [out, setOut] = useState("");
  const [busy, setBusy] = useState(false);
  async function send() {
    setBusy(true); setOut("");
    const res = await fetch("/api/rag-reranker", {
