...
/**
 * Rag Web Scrape — AI recipe
 * Talks to /api/rag-web-scrape (server route) which proxies Lovable AI Gateway.
 */
export function SskRagWebScrape() {
  const [input, setInput] = useState("");
  const [out, setOut] = useState("");
  const [busy, setBusy] = useState(false);
  async function send() {
    setBusy(true); setOut("");
    const res = await fetch("/api/rag-web-scrape", {
...
export default SskRagWebScrape;
