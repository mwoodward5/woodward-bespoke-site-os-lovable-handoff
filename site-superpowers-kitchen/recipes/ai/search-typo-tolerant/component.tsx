...
/**
 * Search Typo Tolerant — AI recipe
 * Talks to /api/search-typo-tolerant (server route) which proxies Lovable AI Gateway.
 */
export function SskSearchTypoTolerant() {
  const [input, setInput] = useState("");
  const [out, setOut] = useState("");
  const [busy, setBusy] = useState(false);
  async function send() {
    setBusy(true); setOut("");
    const res = await fetch("/api/search-typo-tolerant", {
...
export default SskSearchTypoTolerant;
