...
/**
 * Tts Accessible Audio — Voice recipe
 * SSE-streams PCM from /api/tts-accessible-audio into a WebAudio context. Honors prefers-reduced-motion for any visualizer.
 */
export function SskTtsAccessibleAudio() {
  ...
  async function play() {
    ...
    const res = await fetch("/api/tts-accessible-audio", { method:"POST", headers: { "Content-Type":"application/json" }, body: JSON.stringify({ text }) });
...
export default SskTtsAccessibleAudio;
