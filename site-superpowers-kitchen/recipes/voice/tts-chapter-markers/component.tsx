...
/**
 * Tts Chapter Markers — Voice recipe
 * SSE-streams PCM from /api/tts-chapter-markers into a WebAudio context. Honors prefers-reduced-motion for any visualizer.
 */
export function SskTtsChapterMarkers() {
  ...
  async function play() {
    ...
    const res = await fetch("/api/tts-chapter-markers", { method:"POST", headers: { "Content-Type":"application/json" }, body: JSON.stringify({ text }) });
...
export default SskTtsChapterMarkers;
