...
/**
 * Voice Echo Cancel — Voice recipe
 * SSE-streams PCM from /api/voice-echo-cancel into a WebAudio context. Honors prefers-reduced-motion for any visualizer.
 */
export function SskVoiceEchoCancel() {
  ...
  async function play() {
    ...
    const res = await fetch("/api/voice-echo-cancel", { method:"POST", headers: { "Content-Type":"application/json" }, body: JSON.stringify({ text }) });
...
export default SskVoiceEchoCancel;
