...
/**
 * Voice Loop Chat — Voice recipe
 * SSE-streams PCM from /api/voice-loop-chat into a WebAudio context. Honors prefers-reduced-motion for any visualizer.
 */
export function SskVoiceLoopChat() {
  ...
  async function play() {
    ...
    const res = await fetch("/api/voice-loop-chat", { method:"POST", headers: { "Content-Type":"application/json" }, body: JSON.stringify({ text }) });
...
export default SskVoiceLoopChat;
